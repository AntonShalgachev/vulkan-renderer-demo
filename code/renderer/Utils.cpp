...
{
    image = std::make_unique<vkr::Image>(app.getDevice(), width, height, format, tiling, usage);
    imageMemory = std::make_unique<vkr::DeviceMemory>(app.getDevice(), app.getPhysicalDevice(), image->getMemoryRequirements(), properties);
